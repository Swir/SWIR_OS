...
APP = Path('system/apps/swir-photo-studio.py')
DOC = Path('system/SWIR-NATIVE-PHOTO-STUDIO-0.1.md')
WORKFLOW = Path('.github/workflows/system-native-photo-studio.yml')
...
''')
one('\n\nclass PhotoPolicyError(RuntimeError):', '\n\n' + locale_block + 'class PhotoPolicyError(RuntimeError):')
one(
...
one('        window.set_title("SWIR Photo Studio")', '        window.set_title(self.t("window_title"))')
one('        window.add_css_class("swir-app")', '        window.add_css_class("swir-app")\n        window.set_direction(Gtk.TextDirection.RTL if self.locale.text_direction == "rtl" else Gtk.TextDirection.LTR)')
one('        brand = Gtk.Label(label="◈  SWIR PHOTO STUDIO")', '        brand = Gtk.Label(label=self.t("brand"))')
one(
...
many('            button = Gtk.Button(label=label)\n            button.add_css_class("swir-button")', '            button = self._track_control(Gtk.Button(label=label), label)\n            button.add_css_class("swir-button")', 2)
one('        self.undo_button = Gtk.Button(label="Undo")', '        self.undo_button = self._track_control(Gtk.Button(label=self.t("undo")), self.t("undo"))')
one('        self.redo_button = Gtk.Button(label="Redo")', '        self.redo_button = self._track_control(Gtk.Button(label=self.t("redo")), self.t("redo"))')
one('        crop.append(self._label("Crop x"))', '        crop.append(self._label(self.t("crop_x")))')
one('        crop_button = Gtk.Button(label="Apply crop")', '        crop_button = self._track_control(Gtk.Button(label=self.t("apply_crop")), self.t("apply_crop"))')
one(
...
one('        annotate.append(self._label("Text"))\n        self.text_entry = Gtk.Entry()\n        self.text_entry.set_placeholder_text("SWIR TEXT")', '        annotate.append(self._label(self.t("text")))\n        self.text_entry = self._track_control(Gtk.Entry(), self.t("text"))\n        self.text_entry.set_placeholder_text(self.t("text_placeholder"))')
one('        annotate.append(self._label("scale"))', '        annotate.append(self._label(self.t("scale")))')
one('        text_button = Gtk.Button(label="Add text")', '        text_button = self._track_control(Gtk.Button(label=self.t("add_text")), self.t("add_text"))')
one('        line.append(self._label("width"))', '        line.append(self._label(self.t("width")))')
one('        line_button = Gtk.Button(label="Draw cyan line")', '        line_button = self._track_control(Gtk.Button(label=self.t("draw_line")), self.t("draw_line"))')
one('        self.info_label = Gtk.Label(label="Open a local image to begin")', '        self.info_label = Gtk.Label(label=self.t("empty_info"))')
one('        self.status_label = Gtk.Label(label="Non-destructive editing • source unchanged • explicit export copy")', '        self.status_label = Gtk.Label(label=self.t("safe_status"))')
one('            self._set_status("Only local image files are accepted.")', '            self._set_status(self.t("only_local"))')
one('            self._set_status("Open an image first.")', '            self._set_status(self.t("open_first"))')
one('        dialog = Gtk.FileDialog(title="Open image in SWIR Photo Studio")', '        dialog = Gtk.FileDialog(title=self.t("open_dialog"))')
one('        dialog = Gtk.FileDialog(title="Export edited copy")', '        dialog = Gtk.FileDialog(title=self.t("export_dialog"))')
one('        self._set_status(f"Opened read-only: {path.name}")', '        self._set_status(self.t("opened_readonly", name=path.name))')
one(
...
old = 'The Wayland E2E contract emits `swir.native-photo-studio-runtime-evidence/0.3` and fails closed unless one mapped GTK4 session verifies:\n\n- source immutability;'
new = 'The Wayland E2E contract emits `swir.native-photo-studio-runtime-evidence/0.4` and fails closed unless one mapped GTK4 session verifies:\n\n- the owner-only SWIR language preference drives reviewed English, Polish and Norwegian Bokmål primary controls with deterministic English fallback, applied LTR/RTL direction, keyboard-focusable controls and localized tooltips;\n- source immutability;'
if docs.count(old) != 1:
    raise SystemExit('Photo Studio runtime evidence documentation anchor mismatch')
DOC.write_text(docs.replace(old, new, 1), encoding='utf-8')

wf = WORKFLOW.read_text(encoding='utf-8')


def wone(old: str, new: str) -> None:
    global wf
    count = wf.count(old)
    if count != 1:
        raise SystemExit(f'expected one Photo workflow match, found {count}: {old[:120]!r}')
    wf = wf.replace(old, new, 1)


def wmany(old: str, new: str, expected: int) -> None:
    global wf
    count = wf.count(old)
    if count != expected:
        raise SystemExit(f'expected {expected} Photo workflow matches, found {count}: {old[:120]!r}')
    wf = wf.replace(old, new)


wmany("      - 'system/apps/swir-photo-studio.py'\n      - 'system/apps/swir-photo-studio.desktop'", "      - 'system/apps/swir-photo-studio.py'\n      - 'system/apps/core_runtime.py'\n      - 'system/apps/swir-photo-studio.desktop'", 2)
wone('          python3 -m py_compile system/apps/swir-photo-studio.py system/session/swir-shell.py', '          python3 -m py_compile system/apps/swir-photo-studio.py system/apps/core_runtime.py system/session/swir-shell.py')
wone("          grep -F 'swir.native-photo-studio-runtime-evidence/0.3' system/apps/swir-photo-studio.py >/dev/null", "          grep -F 'swir.native-photo-studio-runtime-evidence/0.4' system/apps/swir-photo-studio.py >/dev/null\n          grep -F 'from core_runtime import UserSettingsStore, normalize_language_tag' system/apps/swir-photo-studio.py >/dev/null\n          grep -F 'catalog = requested if requested in _TRANSLATIONS else \"en\"' system/apps/swir-photo-studio.py >/dev/null")
wone('          sudo install -m 0755 system/apps/swir-photo-studio.py /usr/local/bin/swir-photo-studio', '          sudo install -m 0755 system/apps/swir-photo-studio.py /usr/local/bin/swir-photo-studio\n          sudo install -d -m 0755 /usr/local/lib/swir\n          sudo install -m 0644 system/apps/core_runtime.py /usr/local/lib/swir/core_runtime.py')
wone('          RUNTIME="${{ runner.temp }}/swir-photo-runtime"\n          EXPORT_DIR="$RUNTIME/export"\n          mkdir -p "$RUNTIME" "$EXPORT_DIR"\n          chmod 0700 "$RUNTIME" "$EXPORT_DIR"', '          RUNTIME="${{ runner.temp }}/swir-photo-runtime"\n          EXPORT_DIR="$RUNTIME/export"\n          CONFIG_DIR="$RUNTIME/config"\n          mkdir -p "$RUNTIME" "$EXPORT_DIR" "$CONFIG_DIR/swir"\n          chmod 0700 "$RUNTIME" "$EXPORT_DIR" "$CONFIG_DIR" "$CONFIG_DIR/swir"\n          cat > "$CONFIG_DIR/swir/settings.json" <<\'JSON\'\n          {"schema":"swir.user-settings/0.1","appearance":"dark","language":"pl-PL","clock24h":true,"themeId":"builtin.swir-dark"}\n          JSON\n          chmod 0600 "$CONFIG_DIR/swir/settings.json"\n          export XDG_CONFIG_HOME="$CONFIG_DIR"')
wone('                SWIR_PHOTO_E2E_EXPORT_DIR="$EXPORT_DIR" \\\n            python3 system/apps/swir-photo-studio.py', '                SWIR_PHOTO_E2E_EXPORT_DIR="$EXPORT_DIR" \\\n            /usr/local/bin/swir-photo-studio')
wone("          assert evidence['schema'] == 'swir.native-photo-studio-runtime-evidence/0.3'", "          assert evidence['schema'] == 'swir.native-photo-studio-runtime-evidence/0.4'")
wone("          assert evidence['nativeToolkit'] == 'gtk4-gdkpixbuf'", "          assert evidence['nativeToolkit'] == 'gtk4-gdkpixbuf'\n          assert evidence['localeRequested'] == 'pl-PL' and evidence['localeCatalog'] == 'pl-PL'\n          assert evidence['localeFallback'] is False and evidence['textDirection'] == 'ltr' and evidence['textDirectionApplied'] is True\n          assert evidence['localizedWindowTitle'] == 'Studio zdjęć SWIR' and evidence['localizedOpenLabel'] == 'Otwórz obraz' and evidence['localizedUndoLabel'] == 'Cofnij'\n          assert evidence['localizedSurfaceVerified'] is True and evidence['accessibilityFocusVerified'] is True and evidence['accessibilityTooltipsVerified'] is True")
