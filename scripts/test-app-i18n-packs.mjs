...
const context={window:{SwirI18n:{registerMessages(locale,messages){registered.set(locale,messages);}}}};
vm.createContext(context);
for(const file of ['swir-app-locales-west.js','swir-app-locales-global.js','swir-app-locales-world.js']) vm.runInContext(fs.readFileSync(file,'utf8'),context,{filename:file});
...
for(const page of ['swir-files.html','swir-store.html','swir-updates.html','swir-settings.html']) assert(helper.includes(`'${page}'`),`helper missing ${page}`);
...
const order=['swir-app-locales-west.js','swir-app-locales-global.js','swir-app-locales-world.js','swir-app-i18n-host.js'].map(name=>bootstrap.indexOf(name));
...
for(const file of ['swir-app-locales-west.js','swir-app-locales-global.js','swir-app-locales-world.js','swir-app-i18n.js','swir-app-i18n-host.js']) assert(sw.includes(`./${file}`),`service worker must cache ${file}`);
