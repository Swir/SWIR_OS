...
![Roadmap](https://img.shields.io/badge/ROADMAP-92.4%25-02050A?style=for-the-badge&logoColor=62E5FF)
![Completed](https://img.shields.io/badge/DONE-61%2F66-02050A?style=for-the-badge&logoColor=62E5FF)
...
<img width="100%" src="assets/readme/progress-card.svg" alt="SWIR OS roadmap progress — 92.4%, 61 of 66 verified deliverables" />

**Measured scope:** full Web → Desktop → System `Version roadmap`. **Project progress:** 61/66 = **92.4%**. **Release readiness:** not ready; physical Live USB qualification, supported-device fwupd/LVFS validation, Desktop signing/update-feed work, the Konofix replacement and other release gates remain open.
...
|---|---|
| 🧩 **Installable Web widgets** | The official Web catalog now includes verified installable Clock and System widgets with a bounded shell host. The host only renders committed official package records whose protected identity and Web-only compatibility match the allowlist; widgets are local/read-only, permission-free and do not create native-support claims. |
...
| 🖥️ **Desktop bridge** | Windows Desktop Host exposes native filesystem, app-data, account/session, process/service, device/network, clipboard, tray, shortcuts, file-association and notification adapters. The notification path now also carries bounded declarative actions, native action invocation and versioned validated persistence with safe fallback. |
...
| 🔧 **Hardware & drivers** | PCI/USB inventory, Hardware Catalog, Driver Center runtime, Linux in-tree drivers and `linux-firmware` are verified foundations. The allowlisted official-vendor repository path now binds reviewed policy/evidence to Driver Center activation, disposable-VM recovery, and disabled-by-default final-image trust staging; physical vendor-hardware qualification remains separate. Supported Driver Center package/fwupd mutation routes add a private parent journal; direct kernel-module mutation remains disabled. |
...
| 🎨 **Shell themes** | Installable data-only `.swirtheme` packages now support bounded visual tokens, accessibility contrast validation, owner-only atomic storage, Settings import/select/reset and fail-safe recovery to the built-in SWIR Dark theme. Theme packages cannot inject CSS selectors, scripts, commands, URLs or privileged code. |
...
- Real supported-device fwupd/LVFS mutation, physical vendor-driver qualification, direct module mutation and any rollback mechanism not proven by the child service remain outside the completed journaled-transaction gate.
