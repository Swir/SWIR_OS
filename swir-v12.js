...
(() => {
  ...
  function legacyVFS(){const v=readJSON(VFS_KEY,[]);return Array.isArray(v)?v:[]}
  async function mutateVFS(mutator){
    if(!window.SwirFileStorage)throw new Error('File storage adapter unavailable');
    const legacy=legacyVFS();const state=await window.SwirFileStorage.load({legacyItems:legacy});const items=Array.isArray(state?.items)?state.items.map(x=>({...x})):[];
    mutator(items);await window.SwirFileStorage.save(items);if(window.SwirFileStorage.info().persistent)localStorage.removeItem(VFS_KEY);return items;
  }
  ...
  async function newFolder(){const name=prompt('Virtual folder name:','New Folder');if(!name?.trim())return;try{await mutateVFS(items=>items.push({id:uid(),type:'folder',name:name.trim(),parent:'root',trashed:false,created:Date.now()}));toast('File Explorer',`Folder “${name.trim()}” created in Documents.`);open('files')}catch(err){toast('File Explorer',`Storage error: ${err?.message||err}`)}}
  ...
  function wireContext(){
    createContextMenu();
    $('#os-shell')?.addEventListener('contextmenu',e=>{if(matchMedia('(max-width:760px)').matches)return;if(e.target.closest('.os-window,.launcher,.taskbar,.v11-side-panel,#lock-screen,#boot-screen'))return;e.preventDefault();showContext(e.clientX,e.clientY)});
    document.addEventListener('pointerdown',e=>{if(context?.classList.contains('open')&&!e.target.closest('#swir-context-menu'))hideContext()});window.addEventListener('blur',hideContext);window.addEventListener('resize',hideContext);
  }
  ...
  function wireTerminalCommands(){document.addEventListener('keydown',e=>{const input=e.target;if(!(input instanceof HTMLInputElement)||input.id!=='terminal-input'||e.key!=='Enter')return;const raw=input.value.trim();const cmd=raw.toLowerCase().split(/\s+/)[0];const custom={files:'files',explorer:'files',notes:'notes',calc:'calc',calculator:'calc',music:'player',player:'player',monitor:'monitor',sysinfo:'monitor',store:'store'};if(raw.toLowerCase()==='help'){setTimeout(()=>{printTerminal('SWIR OS 1.2 commands: files | notes | calc | player | monitor | lock','term-accent')},20);return}if(cmd==='lock'){e.stopImmediatePropagation();e.preventDefault();printTerminal(`swir@neon-core:~$ ${raw}`,'term-ok');printTerminal('Locking creator session...','term-accent');input.value='';$('#quick-lock')?.click();return}if(custom[cmd]){e.stopImmediatePropagation();e.preventDefault();printTerminal(`swir@neon-core:~$ ${raw}`,'term-ok');printTerminal(`Opening ${app(custom[cmd])?.title||custom[cmd]}...`,'term-accent');input.value='';open(custom[cmd])}},true)}
