...
# Tiny fixed 5x7 bitmap font. Text annotation intentionally supports a bounded,
# deterministic ASCII subset rather than loading arbitrary fonts/plugins.
FONT_5X7: Final = {
...
def _draw_line(
    ...
    *,
    width: int = 3,
    color: tuple[int, int, int] = ANNOTATION_COLOR,
) -> GdkPixbuf.Pixbuf:
    ...
    radius_before = (width - 1) // 2
    radius_after = width // 2
    ...
    for char_index, char in enumerate(normalized):
        ...
        for row_index, row in enumerate(FONT_5X7[char]):
            for column_index, bit in enumerate(row):
                if bit != "1":
                    continue
                base_x = origin_x + column_index * scale
                base_y = y + row_index * scale
...
class SwirPhotoStudio(Gtk.Application):
    def __init__(self) -> None:
        ...
        self.crop_y: Gtk.SpinButton | None = None
        self.crop_w: Gtk.SpinButton | None = None
        self.crop_h: Gtk.SpinButton | None = None
        self.text_entry: Gtk.Entry | None = None
        self.text_x: Gtk.SpinButton | None = None
        self.text_y: Gtk.SpinButton | None = None
        self.text_scale: Gtk.SpinButton | None = None
        self.line_x0: Gtk.SpinButton | None = None
        self.line_y0: Gtk.SpinButton | None = None
        self.line_x1: Gtk.SpinButton | None = None
        self.line_y1: Gtk.SpinButton | None = None
        self.line_width: Gtk.SpinButton | None = None
    ...
    def do_activate(self) -> None:
        ...
        self.crop_y = self._spin(0, 1)
        crop.append(self.crop_y)
        crop.append(self._label("w"))
        self.crop_w = self._spin(1, 1, 1)
        crop.append(self.crop_w)
        ...
        self.text_x = self._spin(0, 1)
        annotate.append(self.text_x)
    ...
    def _sync_geometry_controls(self) -> None:
        width = self.history.current.get_width() if self.history else 1
        height = self.history.current.get_height() if self.history else 1
        ...
        for spin, maximum in ((self.crop_y, max(0, height - 1)), (self.text_y, max(0, height - 1)), (self.line_y0, max(0, height - 1)), (self.line_y1, max(0, height - 1))):
            if spin:
                ...
                spin.set_value(0)
        if self.crop_w:
            self.crop_w.set_range(1, width)
            self.crop_w.set_value(width)
        if self.crop_h:
            self.crop_h.set_range(1, height)
            self.crop_h.set_value(height)
        if self.line_x1:
            self.line_x1.set_value(max(0, width - 1))
        if self.line_y1:
            self.line_y1.set_value(max(0, height - 1))
    ...
    def _crop_rect(self, x: int, y: int, width: int, height: int) -> None:
        ...
        if history is None:
            return
        try:
            result = _crop_pixbuf(history.current, x, y, width, height)
        except PhotoPolicyError as exc:
            self._set_status(f"Crop refused: {exc}")
            return
    ...
    def _crop_from_controls(self) -> None:
        if not all((self.crop_x, self.crop_y, self.crop_w, self.crop_h)):
            return
    ...
    def _annotate_current(self, text: str, x: int, y: int, scale: int) -> None:
        ...
        if history is None:
            return
        try:
            result = _annotate_text(history.current, text, x, y, scale=scale)
        except PhotoPolicyError as exc:
            self._set_status(f"Text refused: {exc}")
            return
        self._apply(result, f"Text annotation added at {x},{y}.")

    def _annotate_from_controls(self) -> None:
        if not all((self.text_entry, self.text_x, self.text_y, self.text_scale)):
            return
    ...
    def _draw_current_line(self, x0: int, y0: int, x1: int, y1: int, width: int) -> None:
        ...
        if history is None:
            return
        try:
            result = _draw_line(history.current, x0, y0, x1, y1, width=width)
        except PhotoPolicyError as exc:
            self._set_status(f"Drawing refused: {exc}")
            return
    ...
    def _run_e2e(self) -> bool:
        ...
        crop_width = max(8, before_crop[0] - 4)
        crop_height = max(12, before_crop[1] - 6)
        crop_x = min(2, before_crop[0] - crop_width)
        crop_y = min(3, before_crop[1] - crop_height)
        self._crop_rect(crop_x, crop_y, crop_width, crop_height)
        ...
        before_text = bytes(self.history.current.get_pixels())
        self._annotate_current("OS", 0, 1, 1)
        after_text = bytes(self.history.current.get_pixels())
        current_w, current_h = self.history.current.get_width(), self.history.current.get_height()
        before_line = bytes(self.history.current.get_pixels())
        self._draw_current_line(0, current_h - 1, current_w - 1, 0, 2)
        after_line = bytes(self.history.current.get_pixels())
        ...
        payload = {
            ...
            "cropVerified": cropped_size == (crop_width, crop_height) and cropped_size[0] < before_crop[0] and cropped_size[1] < before_crop[1],
            "customCropVerified": crop_x >= 0 and crop_y >= 0,
            ...
            "textAnnotationVerified": after_text != before_text,
            "drawingVerified": after_line != before_line,
            "annotationTextBound": MAX_ANNOTATION_CHARS,
            "annotationLineWidthBound": MAX_LINE_WIDTH,
...
def _self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="swir-photo-selftest-") as temp:
        ...
        assert rotated is not None and (rotated.get_width(), rotated.get_height()) == (40, 64)
        ...
        cropped = _crop_pixbuf(history.current, 2, 3, 30, 40)
        assert (cropped.get_width(), cropped.get_height()) == (30, 40)
        try:
        ...
        assert _first_rgb(_transform_pixels(history.current, exposure_stops=0.25)) != base_rgb
        assert _first_rgb(_transform_pixels(history.current, brightness=0.1)) != base_rgb
        assert _first_rgb(_transform_pixels(history.current, contrast=0.2)) != base_rgb
        ...
        text_before = bytes(cropped.get_pixels())
        annotated = _annotate_text(cropped, "SWIR", 0, 2, scale=1)
        assert bytes(annotated.get_pixels()) != text_before
        line_before = bytes(annotated.get_pixels())
        drawn = _draw_line(annotated, 0, 39, 29, 0, width=2)
        assert bytes(drawn.get_pixels()) != line_before
        for bad_text in ("", "A\nB", "snowman ☃", "X" * (MAX_ANNOTATION_CHARS + 1)):
            try:
                _annotate_text(cropped, bad_text, 0, 0, scale=1)
            except PhotoPolicyError:
                pass
            else:
                raise AssertionError("unsafe/unsupported annotation text must be rejected")
        try:
            _draw_line(cropped, -1, 0, 2, 2)
        except PhotoPolicyError:
            pass
        else:
            raise AssertionError("out-of-bounds line coordinate must be rejected")
        try:
            _draw_line(cropped, 0, 0, 2, 2, width=MAX_LINE_WIDTH + 1)
        except PhotoPolicyError:
            pass
        else:
            raise AssertionError("out-of-range line width must be rejected")

        output = root / "copy.jpg"
        _atomic_export(drawn, output)
