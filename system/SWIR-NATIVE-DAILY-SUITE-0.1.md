...
The locale/accessibility closure is now advancing through real application surfaces rather than inventory claims. Calculator, Notes, Network Center and Text Editor consume the bounded per-user SWIR language preference, expose reviewed English/Polish/Norwegian Bokmål chrome with safe English fallback, and carry runtime evidence for localized Wayland surfaces plus keyboard-focus/tool-tip accessibility where those controls apply. Network Center remains read-only, Notes keeps its owner-only atomic storage boundary, and Text Editor retains its no-follow/exclusive-create/inode-identity overwrite protections while localized.

The remaining umbrella completion gate is **suite-wide locale/accessibility depth and final daily-use integration verification across the supported first-party applications**. The verified Calculator/Notes/Network/Text Editor slices reduce that gap but do not complete it; a green inventory/evidence gate must not hide the remaining product-depth work or inflate roadmap progress.
