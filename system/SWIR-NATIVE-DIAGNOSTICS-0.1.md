...
- The application does not invoke `sudo`, `su`, `pkexec`, Polkit or any mutation API. It cannot stop/start services, kill processes, edit configuration or clear logs.
...
`.github/workflows/system-native-diagnostics.yml` compiles the application, checks the fixed-command and no-elevation policy, maps the real GTK4 window on headless Wayland, then validates the emitted runtime evidence. The existing native-core-app workflow continues to cover the process/resource view so the diagnostics addition cannot replace or weaken that baseline.

## Scope boundary

This foundation intentionally does **not** close the umbrella roadmap item for a complete dependable daily-use native application suite. Future repair actions, if introduced, must use explicit privileged brokers with user authorization and a separate auditable contract rather than extending this read-only surface with direct elevation.
