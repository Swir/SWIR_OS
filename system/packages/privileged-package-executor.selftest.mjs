...
assert.equal(PrivilegedPackageExecutorPolicy.aptNonInteractiveAfterAuthorization, true);
...
const calls = [];
const rootOwnedStat = () => ({ isFile: () => true, uid: 0, mode: 0o100755 });
...
assert.deepEqual(calls[0].args, ['/usr/bin/apt-get', '-y', 'install', '--', 'example-editor']);
...
const nonRootExecutor = new GuardedPkexecPackageExecutor({ statSync: () => ({ isFile: () => true, uid: 1000, mode: 0o100755 }), runner });
await assert.rejects(() => nonRootExecutor.execute(request), /not root-owned/);

const writableExecutor = new GuardedPkexecPackageExecutor({ statSync: () => ({ isFile: () => true, uid: 0, mode: 0o100775 }), runner });
